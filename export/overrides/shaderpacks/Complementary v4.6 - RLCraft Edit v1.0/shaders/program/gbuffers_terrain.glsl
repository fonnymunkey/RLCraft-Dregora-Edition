					...
					parallaxShadow, emissive, subsurface, leaves, materialAO);
						...
						if (quarterNdotUp > 0.85) {
							...
							if (noRain < 0.1) {
								...
								float pnormalMultiplier = 0.015;

								vec2 pnormalCoord1 = rainPos + vec2(wind.x, wind.y);
								vec3 pnormalNoise1 = texture2D(noisetex, pnormalCoord1).rgb;
								vec2 pnormalCoord2 = rainPos + vec2(wind.x * -1.0, wind.y * -1.0);
								vec3 pnormalNoise2 = texture2D(noisetex, pnormalCoord2).rgb;
								
								vec2 pnormalCoord3 = rainPos + vec2(wind.x * -1.0, wind.y);
								vec3 pnormalNoise3 = texture2D(noisetex, pnormalCoord3).rgb;
								vec2 pnormalCoord4 = rainPos + vec2(wind.x, wind.y * -1.0);
								vec3 pnormalNoise4 = texture2D(noisetex, pnormalCoord4).rgb;

								pnormalMap += (pnormalNoise1 - vec3(0.5)) * pnormalMultiplier;
								pnormalMap += (pnormalNoise2 - vec3(0.5)) * pnormalMultiplier;
								pnormalMap += (pnormalNoise3 - vec3(0.5)) * pnormalMultiplier;
								pnormalMap += (pnormalNoise4 - vec3(0.5)) * pnormalMultiplier;
