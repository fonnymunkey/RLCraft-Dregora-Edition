...
#if ((defined BLACK_OUTLINE || defined PROMO_OUTLINE) && defined OUTLINE_ON_EVERYTHING && defined END && defined ENDER_NEBULA) || defined WATER_REFRACT || NETHER_REFRACT > 0 && defined NETHER || defined LIGHT_SHAFTS || defined RAINBOW
...
void main() {
	...
	vec3 exInfo = texture2D(colortex2, texCoord.xy).rgb;
	float z0 = texture2D(depthtex0, texCoord.xy).r;
	float z1 = texture2D(depthtex1, texCoord.xy).r;
	bool water=false, moddedfluidShiv=false, moddedfluidShivX=false;
	float glass = exInfo.r;
	glass = 0.0; // Disable Glass Effect For Now
	float mat = exInfo.g * 64.0; // same mat as in gbuffers_water

	if (mat > 0.98 && mat < 1.02) {
		water = true;
	}
	if (mat > 5.98 && mat < 7.02) {
		water = true;
		moddedfluidShiv = (mat > 5.98 && mat < 6.02);
		moddedfluidShivX = (mat > 6.98 && mat < 7.02);
	}
	if (water) {
    ...
    #if defined LIGHT_SHAFTS || defined WATER_REFRACT || NETHER_REFRACT > 0 && defined NETHER || defined RAINBOW
        viewPos = gbufferProjectionInverse * (vec4(texCoord, z0, 1.0) * 2.0 - 1.0);
        viewPos /= viewPos.w;
    #endif

	vec2 refractCoord = texCoord.xy;
	vec2 refractCoord2;
	...
	// water refraction effect
		...
		if (water || glass > 0.5)
		{
			...
			float speed = frametime * (1.0 - glass);
			if (moddedfluidShiv) {
				speed *= 0.25;
			}
			vec3 worldPosM = worldPos.xyz + cameraPosition.xyz;
			float refractSize = 0.005;
			if (moddedfluidShiv) {
				refractSize = 0.002;
			}
			vec2 refractPos = worldPosM.xz * refractSize + worldPosM.y * 0.0025 + 0.0035 * WATER_SPEED * speed;
			...
			float waterCheck = float(texture2D(colortex1, refractCoord).b > 0.999 || moddedfluidShiv || moddedfluidShivX);
			...
			if (z0check >= 0.56 && waterCheck > 0.95 || glass > 0.5) {
                refractCoord = texCoord.xy + refractNoise;
            } else {
                refractCoord = texCoord.xy;
            }
		}
	#endif
	...
	#if defined NETHER && NETHER_REFRACT > 0
		{
			vec3 worldPos = ViewToWorld(viewPos.xyz);
			float speed = frametime;
			vec3 worldPosM = normalize(worldPos) * 0.035;
			vec2 refractPos = worldPosM.xz + worldPosM.y * 0.5 - 0.001 * WATER_SPEED * speed;
			vec2 refractNoise = texture2D(noisetex, refractPos).rg - vec2(0.5);
	...
	// sample nether heatwave refraction
	#if defined NETHER && NETHER_REFRACT > 0
		vec3 color2;
		#if MC_VERSION > 10710
			color2 = texture2D(colortex0, refractCoord2).rgb;
		#else
			//To fix blocky water in Mc 1.7.10 (reason unknown)
			color2 = texture2DLod(colortex0, refractCoord2, 0).rgb;
		#endif
		// mostly use water refraction if rendering water, otherwise heatwave
		color.rgb = mix(color2.rgb, color.rgb, water ? 1.0 : 0.0);
	#endif
