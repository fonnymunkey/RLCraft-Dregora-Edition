                ...
                if (mc_Entity.x < 11060.5) {
                    ...
                    if (mc_Entity.x < 11072.5) {
                        if (mc_Entity.x == 11064) // Command Blocks
                            ...
                            #endif
                            specR = 0.5, specG = 1.0;
						else if (mc_Entity.x == 11069) // Shivs bullshit
                            #ifdef COLORED_LIGHT
                                lightVarying = 2.0,
                            #endif
                            lmCoord.x = min(lmCoord.x, 0.87), specB = 6.15, mat = 20000.0, color.rgb = vec3(0.0, 1.0, 0.0);
                    } else {
						...
						else if (mc_Entity.x == 11079) { // Ice n Fire Lilys
                            #if EMISSIVE_LICHEN > 0
                                #if EMISSIVE_LICHEN == 1
                                    float lightFactor = max(1.0 - lmCoord.y, 0.0);
                                          lightFactor *= lightFactor;
