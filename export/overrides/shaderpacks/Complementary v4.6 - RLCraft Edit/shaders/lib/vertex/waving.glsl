...
float calcWavingLava(vec3 worldPos) {
    float wave = sin(2 * PI * (wavingTime*0.25 + worldPos.x * 0.14 + worldPos.z * 0.07))
                + sin(2 * PI * (wavingTime*0.15 + worldPos.x * 0.10 + worldPos.z * 0.20))
				+ sin(2 * PI * (wavingTime*0.1 + worldPos.x * 0.06 + worldPos.z * 0.32));
    return wave * 0.0125;
}
...
vec3 WavingBlocks(vec3 position, float istopv, float lmCoordY) {
	...
	if (mc_Entity.x == 11079 && (istopv > 0.9 || fract(worldpos.y + 0.005) > 0.01)) { // Ice n Fire Lilys
        wave += calcMove(worldpos, 0.0041, 0.005, 0.0044, 0.0038, 0.0240, 0.0000, vec3(0.6,0.0,0.6), vec3(0.3,0.0,0.3));
	}
    #endif
	
	if (mc_Entity.x == 10 && fract(worldpos.y + 0.05) > 0.1) { // Lava
        wave.y += calcWavingLava(worldpos);
    }
    ...
    wave *= WAVING_INTENSITY * mix(1.0, RAIN_WAVING_INTENSITY, rainStrengthS);
		...
		if (mc_Entity.x != 10) //LAVA should still "wave" underground, like water
...
float WavingThickLiquid(vec3 worldPos, float lmCoordY) {
	float fractY = fract(worldPos.y + cameraPosition.y + 0.005);
	worldPos += cameraPosition.xyz;

    float waterWaveTime = frametime * 0.5;
