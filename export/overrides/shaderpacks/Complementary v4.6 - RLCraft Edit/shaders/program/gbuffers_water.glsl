...
float GetWaterHeightMapShiv(vec3 worldPos, vec3 nViewPos) {
	float verticalOffset = worldPos.y * 0.005;
	...
	vec4 noiseS = vec4(0.5);
	noiseS.r = texture2D(noisetex, (worldPos.xz) / (1.0 * 100) - wind).g;
	noiseS.g = texture2D(noisetex, (worldPos.xz) / (0.6 * 100) + wind).g;
	noiseS.b = texture2D(noisetex, (worldPos.xz) / (0.35 * 100) + wind).g;
	noiseS.a = texture2D(noisetex, (worldPos.xz) / (0.3 * 100) - wind).g;
	noiseS *= - noiseS;
	float noise = noiseS.r * 0.05 
				- noiseS.r * noiseS.g * 2 
				+ noiseS.g * 0.05 
				+ (noiseS.b - noiseS.a) * 0.05;

	noise *= 0.4 * (lmCoord.y*0.9 + 0.1) * 0.42;

    return noise;
}

vec3 GetParallaxWavesShiv(vec3 worldPos, vec3 nViewPos, vec3 viewVector, float lViewPos) {
	vec3 parallaxPos = worldPos;
	
	for(int i = 0; i < 4; i++) {
		float height = (GetWaterHeightMapShiv(parallaxPos, nViewPos) - 0.5);
		parallaxPos.xz += 0.2 * height * viewVector.xy / lViewPos;
	}
	return parallaxPos;
}

vec3 GetWaterNormalShiv(vec3 worldPos, vec3 nViewPos, vec3 viewVector, float lViewPos) {
	vec3 waterPos = worldPos + cameraPosition;
	#ifdef WATER_PARALLAX
		waterPos = GetParallaxWavesShiv(waterPos, nViewPos, viewVector, lViewPos);
	#endif

	float normalOffset = 0.2;

	float h1 = GetWaterHeightMapShiv(waterPos + vec3( normalOffset, 0.0, 0.0), nViewPos);
	float h2 = GetWaterHeightMapShiv(waterPos + vec3(-normalOffset, 0.0, 0.0), nViewPos);
	float h3 = GetWaterHeightMapShiv(waterPos + vec3(0.0, 0.0,  normalOffset), nViewPos);
	float h4 = GetWaterHeightMapShiv(waterPos + vec3(0.0, 0.0, -normalOffset), nViewPos);
...
void main() {
	...
	float moddedfluidShiv  = float(mat > 5.98 && mat < 6.02);
	float moddedfluidShivX = float(mat > 6.98 && mat < 7.02);
	
	exAlbedo = vec3(translucent, mat/64.0, 0.0);
	...
	if (albedocheck > 0.00001) {
		...
		mat3 tbnMatrix = mat3(tangent.x, binormal.x, normal.x,
		...
		#ifdef WATER_WAVES
			if (water + moddedfluidX + moddedfluidShivX > 0.5) {
			...
			if (moddedfluidShiv > 0.5) {
				normalMap = GetWaterNormalShiv(worldPos, nViewPos, viewVector, lViewPos);
				newNormal = clamp(normalize(normalMap * tbnMatrix), vec3(-1.0), vec3(1.0));
		...
		if (water > 0.5 || moddedfluid > 0.5 || moddedfluidShiv > 0.5 || moddedfluidShivX > 0.5 || (translucent > 0.5 && albedo.a < 0.95)) {
			...
			fresnel *= 1.0 - translucent * (1.0 - albedo.a);
				...
				if (reflection.a < 1.0) {
					...
					#ifdef OVERWORLD
						float specular = 0.0;
						if (water > 0.5 || moddedfluidShivX > 0.5) {
							#if WATER_TYPE >= 1
				...
				if (moddedfluidShiv > 0.5) {
				albedo.rgb = mix(albedo.rgb, reflection.rgb*0.5, fresnel);
				}
				else {
	...
	gl_FragData[2] = vec4(exAlbedo, 1.0);
}
...
void main() {
	...
	if (mc_Entity.x == 886) { // Modded Fluid Same viscosity of water but Keeps Texture AND sun/moon/sky reflections
		#ifdef WATER_DISPLACEMENT
			position.y += WavingWater(position.xyz, lmCoord.y);
		#endif
		mat = 7.0;
	}
	if (mc_Entity.x == 887) { // Modded Fluid Slower Water Waves And Keeps Texture
		#ifdef WATER_DISPLACEMENT
			position.y += WavingThickLiquid(position.xyz, lmCoord.y)*0.5;
		#endif
		mat = 6.0;
	}
